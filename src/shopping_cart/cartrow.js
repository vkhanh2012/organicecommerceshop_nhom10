...
export function cartRow(item) {
  return `
    <div class="grid grid-cols-[minmax(0,1fr)_100px] items-center gap-3 border-t border-neutral-100 px-5 py-3 sm:grid-cols-[minmax(260px,1fr)_100px_150px_100px_24px] sm:px-6 lg:py-[9px] xl:grid-cols-[371px_102px_174px_100px_24px] xl:px-5">
      ...
      <span class="hidden text-base leading-6 text-neutral-900 sm:block">${money(item.price)}</span>
      ...
      <strong class="hidden text-base font-medium leading-6 sm:block">${money(item.price * item.quantity)}</strong>
      ...
      <div class="col-span-2 flex justify-between text-base leading-6 sm:hidden">
