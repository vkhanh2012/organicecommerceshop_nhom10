  ...
  return `
    <aside class="h-fit rounded-lg border border-neutral-100 bg-white p-6">
      ...
      <div class="mt-2 divide-y divide-neutral-100">
        ...
        <button data-action="checkout" class="mt-2 w-full cursor-pointer rounded-full bg-[#edfff0] px-6 py-4 text-sm font-semibold text-primary hover:bg-primary hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed" ${cart.length ? "" : "disabled"}>
        Proceed to checkout
      </button>
    </aside>`;
}
