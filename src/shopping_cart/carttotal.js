...
export function cartTotal(cart) {
  ...
  return `
    <aside class="h-fit rounded-lg border border-neutral-100 bg-white p-6">
      ...
      <button class="mt-2 w-full cursor-pointer rounded-full bg-[#edfff0] px-6 py-4 text-sm font-semibold text-primary hover:bg-primary hover:text-white" ${cart.length ? "" : "disabled"}>Proceed to checkout</button>
    </aside>`;
}
