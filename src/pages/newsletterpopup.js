...
export function renderNewsletterPopup() {
  ...
  return `
    <div class="fixed inset-0 z-[120] flex items-center justify-center bg-black/80 p-4" data-newsletter-overlay>
      <section
        class="relative grid max-h-[92vh] w-full max-w-[872px] overflow-y-auto rounded-lg bg-white shadow-[0_12px_24px_rgba(0,0,0,.12)] md:h-[400px] md:grid-cols-[374px_minmax(0,1fr)] md:overflow-hidden"
      ...
      >
        <button
          type="button"
          class="absolute right-3 top-3 z-10 flex h-9 w-9 cursor-pointer items-center justify-center rounded-full text-2xl text-neutral-700 transition-colors hover:bg-neutral-100 hover:text-neutral-900"
        ...
        <div class="flex flex-col justify-center px-6 pb-7 pt-5 text-center sm:px-10 md:justify-start md:pb-10 md:pl-[30px] md:pr-10 md:pt-[49px]">
          ...
          <p class="mx-auto mt-3 max-w-[410px] text-sm leading-6 text-neutral-400 md:text-base">
            ...
            <span class="font-semibold text-warning">20% money</span>
            with discount code today.
          </p>

          <form class="mt-6 flex w-full flex-col gap-3 sm:flex-row sm:gap-0" data-newsletter-form>
            <input
              type="email"
              required
              name="email"
              placeholder="Enter your email"
              class="h-12 min-w-0 flex-1 rounded-full border border-neutral-100 px-5 text-sm text-neutral-700 outline-none focus:border-primary sm:rounded-r-none"
            >
            <button
              type="submit"
...
export function bindNewsletterPopupEvents(root, onClose) {
  const overlay = root.querySelector("[data-newsletter-overlay]");
  const popup = root.querySelector("[data-newsletter-popup]");
  const hideCheckbox = root.querySelector("[data-newsletter-hide]");

  function closePopup() {
    onClose(Boolean(hideCheckbox?.checked));
  }

  root.querySelector("[data-newsletter-close]")?.addEventListener("click", closePopup);

  overlay?.addEventListener("click", (event) => {
    if (!popup?.contains(event.target)) closePopup();
  });

  root.querySelector("[data-newsletter-form]")?.addEventListener("submit", (event) => {
...
export function initNewsletterPopupPage() {
  if (sessionStorage.getItem(SESSION_KEY) || localStorage.getItem(HIDE_KEY)) return;

  const root = document.createElement("div");
  root.setAttribute("data-newsletter-root", "");
  root.innerHTML = renderNewsletterPopup();
  document.body.append(root);
  document.body.classList.add("overflow-hidden");
  sessionStorage.setItem(SESSION_KEY, "true");

  bindNewsletterPopupEvents(root, (doNotShowAgain) => {
    if (doNotShowAgain) localStorage.setItem(HIDE_KEY, "true");
