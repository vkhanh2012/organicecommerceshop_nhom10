...
const CLASS = {
  ...
  imageWrap: "relative aspect-square rounded-md overflow-hidden bg-white flex items-center justify-center mb-3",
  image: "w-full h-full object-cover",
  tags: "absolute top-2 left-2 z-10 flex gap-1",
  tagSale: "bg-error text-white text-[11px] font-semibold font-poppins px-2 py-1 rounded",
  tagBest: "bg-sky-500 text-white text-[11px] font-semibold font-poppins px-2 py-1 rounded",
  actions: "absolute top-2 right-2 z-10 flex flex-col gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity",
  actionBtn: "w-8 h-8 md:w-9 md:h-9 rounded-full bg-white shadow flex items-center justify-center text-neutral-700 hover:bg-primary hover:text-white transition-colors",
  body: "px-1",
  name: "font-poppins text-sm text-neutral-900 mb-1 truncate transition-colors md:group-hover:text-primary",
...
export function renderProductCard({
  ...
  const starsHtml = Array.from({ length: 5 })
    .map((_, i) => `<span class="${i < rating ? "text-warning" : "text-neutral-200"}">${iconStar(i < rating)}</span>`)
    .join("");
