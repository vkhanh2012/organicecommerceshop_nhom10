export function renderDescriptionTab(product = {}) {
  ...
  const paragraphsHtml = paragraphs
    .map(pText => `<p class="w-full text-neutral-500 text-sm font-normal leading-[21px]">${pText}</p>`)
    .join("");
  ...
  const featuresHtml = featureList.map(feat => `
    <div class="flex items-center gap-2 py-0.5">
      <div class="w-5 h-5 bg-primary rounded-full flex items-center justify-center shrink-0">
        <svg width="10" height="8" viewBox="0 0 10 8" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M9 1L3.5 6.5L1 4" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </div>
      <span class="min-w-0 text-neutral-500 text-sm font-normal leading-[21px]">${feat}</span>
    </div>
  `).join("");

  const sideImage = product.videoImage || product.mainImage || product.image || "https://placehold.co/536x300";
