export function renderImage(product) {
  const thumbnailsHtml = product.thumbnails.map((thumb, index) => {
    ...
    return `
      <div data-action="select-thumb" data-src="${thumb}" class="thumbnail-item w-[80px] h-[90px] border ${isSelected ? 'border-primary' : 'border-neutral-200'} hover:border-primary rounded-lg overflow-hidden p-1 bg-white cursor-pointer transition-all flex items-center justify-center">
  ...
  return /*html*/ `
    ...
    <div id="product-gallery" class="lg:col-span-6 flex flex-col sm:flex-row gap-4">
      <div class="flex sm:flex-col items-center gap-3 order-2 sm:order-1 shrink-0">
        <div class="text-neutral-400 hover:text-neutral-900 cursor-pointer p-1">
      ...
      <div class="w-full aspect-square bg-white border border-neutral-100 rounded-2xl flex items-center justify-center p-6 order-1 sm:order-2">
