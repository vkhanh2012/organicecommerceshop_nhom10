...
export function renderProductInfo(product = {}) {
  ...
  const tagsHtml = tagsList
    .map((tag) => {
      ...
      return `<a href="${tagLink}" class="text-neutral-500 hover:text-primary transition-colors leading-5">${tagName}</a>`;
    })
    .join(`<span class="text-neutral-400 font-normal mr-1">,</span>`);

  return /*html*/ `
    <!-- CỘT PHẢI: KHỐI THÔNG TIN SẢN PHẨM PHÙ HỢP CÁC BIẾN CSS FIGMA / TAILWIND @THEME -->
    <div class="lg:col-span-6 flex flex-col justify-start gap-6 font-['Poppins'] select-none pl-[20px]">
        ...
        <div class="flex flex-col gap-3">
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            ${
              product.inStock !== false
                ? `
                  <span class="bg-instock/20 text-text-instock text-sm font-normal px-2.5 py-1 rounded-sm leading-5">
                    In Stock
                  </span>
                `
                : ""
          ...
          <div class="flex items-center gap-3 text-sm flex-wrap">
            <div class="flex items-center gap-1">
              <div class="flex items-center text-star gap-0.5">
                ${starsHtml}
              </div>
              ...
              product.originalPrice || product.oldPrice
                ? `
                  <span class="text-neutral-400 text-xl font-normal line-through leading-8 mr-1">
                    $${Number(product.originalPrice || product.oldPrice || 48.0).toFixed(2)}
                  </span>
                `
                : ""
            }
            <span class="text-primary-dark text-2xl lg:text-[28px] font-medium leading-9">
              $${Number(product.currentPrice || product.price || 17.28).toFixed(2)}
            </span>
          </div>

          ${
            product.discountLabel || product.saleTag
              ? `
                <span class="bg-error/10 text-error text-sm font-medium px-2.5 py-[3px] rounded-pill leading-5">
                  ${product.discountLabel || product.saleTag || "64% Off"}
                </span>
              `
              : ""
          }
      ...
      <div class="flex flex-col gap-4 -mt-4">
        <div class="w-full flex items-center justify-between text-sm flex-wrap gap-4 py-3 border-b border-neutral-100">
          <!-- BRAND -->
          <div class="flex items-center gap-2">
            <span class="text-neutral-500">Brand:</span>
            ${
              product.brandLogo
                ? `<img src="${product.brandLogo}" alt="${product.brand || 'Brand'}" class="h-8 w-auto" />`
        ...
        <!-- MÔ TẢ -->
        <p class="text-sm text-neutral-500 leading-5 font-normal max-w-[568px] -mt-2">
      ...
      <div class="w-full py-4 border-y border-neutral-200 flex flex-col sm:flex-row items-center justify-center gap-3">
        ...
        <button
          ...
          type="button"
          class="w-full sm:w-96 h-[51px] bg-primary hover:bg-primary-dark text-white font-semibold rounded-pill flex items-center justify-center gap-4 transition-colors cursor-pointer px-10"
        >
